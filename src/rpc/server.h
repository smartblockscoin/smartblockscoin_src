...
/**
 * Smartblockscoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZsbcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzsbcseed(const UniValue& params, bool fHelp);
extern UniValue getzsbcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzsbc(const UniValue& params, bool fHelp);
extern UniValue dzsbcstate(const UniValue& params, bool fHelp);
