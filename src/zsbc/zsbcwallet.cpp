...
CzSBCWallet::CzSBCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsbc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSBCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSBCSeed_deprecated()) {
                    LogPrintf("%s: Updated zSBC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsbc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsbc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSBCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSBCCount(nCountLastUsed);
    else if (!walletdb.ReadZSBCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSBCWallet::Lock()
{
    seedMaster = 0;
}

void CzSBCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSBCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSBC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSBCWallet::LoadMintPoolFromDB()
{
...
void CzSBCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSBCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSBCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsbcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSBCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSBC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsbcTracker which also adds to database
    pwalletMain->zsbcTracker->Add(dMint, true);
...
void CzSBCWallet::SeedToZSBC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSBCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSBCWallet::GenerateDeterministicZSBC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSBCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSBCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSBCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
