...
CzSBCTracker::CzSBCTracker(std::string strWalletFile)
{
...
CzSBCTracker::~CzSBCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSBCTracker::Init()
{
...
bool CzSBCTracker::Archive(CMintMeta& meta)
{
...
bool CzSBCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSBCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSBCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSBCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSBCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSBCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSBCTracker::GetSerialHashes()
{
...
CAmount CzSBCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSBCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSBCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSBCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSBCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSBCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSBCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSBCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSBCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSBCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSBCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSBCWallet* zSBCWallet)
{
    bool iszSBCWalletInitialized = (NULL != zSBCWallet);
    CMintMeta meta;
    ...
    if (! iszSBCWalletInitialized)
        zSBCWallet = new CzSBCWallet(strWalletFile);
    meta.isSeedCorrect = zSBCWallet->CheckSeed(dMint);
    if (! iszSBCWalletInitialized)
        delete zSBCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSBCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSBCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSBCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSBCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSBCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSBCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSBCWallet* zSBCWallet = new CzSBCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSBCWallet);
        }
        delete zSBCWallet;
        LogPrint("zero", "%s: added %d dzsbc from DB\n", __func__, listDeterministicDB.size());
    }
