...
#include "ui_zsbccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zsbcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsbcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSBC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSBC seed used to mint this zSBC is not the same as currently hold in the wallet";
            else
