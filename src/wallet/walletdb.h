...
public:
    ...
    bool WriteZSBCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSBCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSBCSeed_deprecated(uint256& seed);
    bool EraseZSBCSeed();
    bool EraseZSBCSeed_deprecated();

    bool WriteZSBCCount(const uint32_t& nCount);
    bool ReadZSBCCount(uint32_t& nCount);
