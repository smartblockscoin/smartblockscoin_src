...
// The zSBC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSBC is a hash of the serial
...
{
    //Create an output returning the zSBC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSBCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSBC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSBC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSBCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSBC output", __func__);
...
{
    CzSBCTracker* zsbcTracker = pwallet->zsbcTracker.get();
    CMintMeta meta;
    if (!zsbcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zsbcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SBC stake is the outpoint
